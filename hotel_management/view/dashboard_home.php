...
// Các biến tổng số lượng
$totalPhong = isset($GLOBALS['totalPhong']) ? $GLOBALS['totalPhong'] : 0;
$totalKhach = isset($GLOBALS['totalKhach']) ? $GLOBALS['totalKhach'] : 0;
$totalNV = isset($GLOBALS['totalNV']) ? $GLOBALS['totalNV'] : 0;
$totalDV = isset($GLOBALS['totalDV']) ? $GLOBALS['totalDV'] : 0;
$totalKM = isset($GLOBALS['totalKM']) ? $GLOBALS['totalKM'] : 0; 
?>
...
<style>
  ...
  .stats-number { font-size: 2.5rem; font-weight: bold; margin: 5px 0; line-height: 1; }
  .stats-label { font-size: 1rem; font-weight: 500; text-transform: uppercase; white-space: nowrap; }
  
  /* Màu sắc các ô */
  .bg-blue { background-color: #2563eb; } 
  .bg-green { background-color: #398357; } 
  .bg-yellow { background-color: #fac042; } 
  .bg-cyan { background-color: #4dbce9; }   
  .bg-purple { background-color: #6f42c1; }
  
  /* Thao tác nhanh */
  .quick-action-card { background: #fff; border: 1px solid #dee2e6; border-radius: 8px; padding: 20px 10px; text-align: center; cursor: pointer; transition: all 0.3s ease; height: 100%; display: flex; flex-direction: column; align-items: center; justify-content: center; }
  .quick-action-card:hover { transform: translateY(-5px); box-shadow: 0 5px 15px rgba(0,0,0,0.1); border-color: #2563eb; }
  .quick-action-icon { font-size: 2.5rem; color: #2563eb; margin-bottom: 10px; }
  .quick-action-text { font-weight: 500; color: #333; font-size: 0.95rem; }
...
<div class="container-fluid pb-5">
  ...
  <div class="row g-3 mb-4 mt-2 justify-content-center">
      <div class="col-xl-2 col-md-4 col-6"><div class="stats-card bg-blue"><i class="fas fa-door-open stats-icon"></i><div class="stats-label">Phòng</div><div class="stats-number"><?php echo $totalPhong; ?></div></div></div>
      <div class="col-xl-2 col-md-4 col-6"><div class="stats-card bg-green"><i class="fas fa-users stats-icon"></i><div class="stats-label">Khách hàng</div><div class="stats-number"><?php echo $totalKhach; ?></div></div></div>
      <div class="col-xl-2 col-md-4 col-6"><div class="stats-card bg-yellow"><i class="fas fa-user-tie stats-icon"></i><div class="stats-label">Nhân viên</div><div class="stats-number"><?php echo $totalNV; ?></div></div></div>
      <div class="col-xl-2 col-md-4 col-6"><div class="stats-card bg-cyan"><i class="fas fa-concierge-bell stats-icon"></i><div class="stats-label">Dịch vụ</div><div class="stats-number"><?php echo $totalDV; ?></div></div></div>
      <div class="col-xl-2 col-md-4 col-6"><div class="stats-card bg-purple"><i class="fas fa-tags stats-icon"></i><div class="stats-label">Khuyến Mãi</div><div class="stats-number"><?php echo $totalKM; ?></div></div></div>
  </div>

  <div class="section-title"><i class="fas fa-bolt"></i> Thao tác nhanh</div>
  <div class="row g-3 justify-content-center">
    <div class="col-xl-2 col-md-4 col-6"><div class="quick-action-card" data-bs-toggle="modal" data-bs-target="#addRoomModal"><i class="fas fa-door-open quick-action-icon"></i><span class="quick-action-text">Thêm phòng</span></div></div>
    <div class="col-xl-2 col-md-4 col-6"><div class="quick-action-card" data-bs-toggle="modal" data-bs-target="#addCustomerModal"><i class="fas fa-user-plus quick-action-icon"></i><span class="quick-action-text">Thêm khách</span></div></div>
    <div class="col-xl-2 col-md-4 col-6"><div class="quick-action-card" data-bs-toggle="modal" data-bs-target="#addStaffModal"><i class="fas fa-user-tie quick-action-icon"></i><span class="quick-action-text">Thêm nhân viên</span></div></div>
    <div class="col-xl-2 col-md-4 col-6"><div class="quick-action-card" data-bs-toggle="modal" data-bs-target="#addServiceModal"><i class="fas fa-concierge-bell quick-action-icon"></i><span class="quick-action-text">Thêm dịch vụ</span></div></div>
    <div class="col-xl-2 col-md-4 col-6"><div class="quick-action-card" data-bs-toggle="modal" data-bs-target="#addKMModal"><i class="fas fa-tags quick-action-icon"></i><span class="quick-action-text">Thêm khuyến mãi</span></div></div>
  </div>

  <div class="section-title"><i class="fas fa-chart-line"></i> Biểu đồ phân tích</div>
  <div class="row g-3 mb-4">
    <div class="col-lg-8 col-md-7"><div class="card-dashboard"><h6 class="text-center text-muted mb-3">Doanh thu theo tháng (VNĐ)</h6><div style="height: 300px;"><canvas id="revenueChart"></canvas></div></div></div>
    <div class="col-lg-4 col-md-5"><div class="card-dashboard"><h6 class="text-center text-muted mb-3">Tỷ lệ lấp đầy phòng</h6><div style="height: 250px; display: flex; justify-content: center;"><canvas id="occupancyChart"></canvas></div><div class="text-center mt-3 pt-2 border-top"><small class="text-muted">Tổng số phòng: <strong class="text-primary fs-5"><?php echo $totalPhong; ?></strong></small></div></div></div>
  </div>
  ...
  <div class="card-dashboard mb-4 p-0 overflow-hidden">
    <div class="table-responsive">
      <table class="table table-hover align-middle mb-0 custom-table">
        <thead><tr><th>Khách</th><th>Phòng</th><th>Loại</th><th>Nhận</th><th>Trả</th><th>Trạng thái</th><th>SĐT</th></tr></thead>
        <tbody>
          <?php if (!empty($recentBookings)): foreach ($recentBookings as $bk): ?>
              <tr>
                ...
                <td><span class="status-badge <?php echo (stripos($bk['TrangThai'],'DaNhan')!==false)?'checkedin':'pending'; ?>"><?php echo htmlspecialchars($bk['TrangThai']); ?></span></td>
                <td><?php echo htmlspecialchars($bk['DienThoai']); ?></td>
              </tr>
            <?php endforeach; else: ?><tr><td colspan="7" class="text-center py-4 text-muted">Không có dữ liệu đặt phòng nào</td></tr><?php endif; ?>
        </tbody>
      </table>
    </div>
  </div>
      ...
      <table class="table table-hover align-middle mb-0 custom-table">
        ...
        <tbody>
          <?php if (!empty($currentOccupied)): ?>
            ...
            <?php endforeach; ?>
          <?php else: ?>
            <tr><td colspan="7" class="text-center py-4 text-muted">Hiện không có phòng nào có khách</td></tr>
          <?php endif; ?>
        </tbody>
      </table>
    </div>
...
<div class="modal fade" id="addKMModal" tabindex="-1"><div class="modal-dialog"><div class="modal-content"><div class="modal-header"><h5 class="modal-title fw-bold">Thêm Khuyến Mãi Mới</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div><form id="kmForm"><div class="modal-body">
    <div class="mb-3"><label class="form-label">Mã KM</label><input name="maKM" class="form-control" required placeholder="VD: KM001"></div>
    <div class="mb-3"><label class="form-label">Tên chương trình</label><input name="tenCT" class="form-control" required></div>
    <div class="mb-3"><label class="form-label">Mức giảm (%)</label><input name="mucGiam" type="number" class="form-control" required min="0" max="100"></div>
    <div class="row">
        <div class="col-6 mb-3"><label class="form-label">Ngày bắt đầu</label><input type="date" name="ngayBatDau" class="form-control" required></div>
        <div class="col-6 mb-3"><label class="form-label">Ngày kết thúc</label><input type="date" name="ngayKetThuc" class="form-control" required></div>
    </div>
</div><div class="modal-footer"><button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Đóng</button><button type="submit" class="btn btn-primary">Lưu</button></div></form></div></div></div>
...
new Chart(document.getElementById('occupancyChart').getContext('2d'), { type: 'doughnut', data: { labels: ['Đang ở','Đã đặt','Trống','Bảo trì'], datasets: [{ data: dataPhong, backgroundColor: ['#28a745','#ffc107','#6c757d','#dc3545'] }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position:'right' } } } });

// --- AJAX FORM (Đã sửa lỗi không lưu được khuyến mãi) ---
$(function(){
  function submitForm(url, data, modalId) {
      $.post(url, data, function(resp){
          // Fix lỗi parse JSON khi server trả về lỗi PHP hoặc chuỗi
          var res;
          try {
              res = (typeof resp === 'object') ? resp : JSON.parse(resp);
          } catch(e) {
              // Nếu không parse được JSON, tức là có lỗi hệ thống hoặc cảnh báo PHP
              console.error("Server response error:", resp);
              alert('Lỗi hệ thống hoặc kết nối! Dữ liệu chưa được lưu.');
              return;
          }
          ...
          if(res.success) { 
              ...
              alert('Thất bại: ' + res.message); // Hiển thị thông báo lỗi chính xác từ server
          }
      }).fail(function() {
          alert("Lỗi kết nối đến máy chủ! Vui lòng kiểm tra lại mạng hoặc đường dẫn Controller.");
      });
  }
  ...
  // Xử lý thêm KM
  $('#kmForm').on('submit', function(e){ e.preventDefault(); submitForm('controller/khuyenmaiController.php', $(this).serialize()+'&action=themKM', '#addKMModal'); });
});
</script>